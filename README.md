...
3. シート名をべた書きしている箇所を直します：ヘッダーの直リンク（`AppHeader.tsx`）と、リンクが1行に収まる表示切り替えの幅（`AppHeader.module.css`）、一覧の予告カードの文言（`CatalogPage.tsx`）、`index.html` の meta description、このREADME冒頭の収録一覧とディレクトリ構成。
